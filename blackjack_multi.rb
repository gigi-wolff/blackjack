...
def remove_player_from_game(player_name,players)
  players.delete(player_name)
end

players = {} #player = {name => hand value}
deck = CARDS.product(SUITS)
deck = (deck*4).shuffle! #play with four decks
dealer_hand = []
dealer_total = 0

players = get_players
players.each {|name,_v| puts "Welcome #{name}!"}
...
players.each do |name,hand_value|
  ...
  if (players[name] > 21) then
    puts "Busted!"
    remove_player_from_game(name,players)
